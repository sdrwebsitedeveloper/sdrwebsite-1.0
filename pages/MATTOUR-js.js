...
export function renderMATTOUR() {
    document.getElementById('app').innerHTML = `
        <div class="container">
            <div class="image-container">
                <img id="matTourImage" alt="MATTOUR Image">
            </div>
            <div id="matTourDescription" class="description">설명글이 여기에 표시됩니다.</div>
            <button id="uploadButton" class="upload-button" style="display:none;">사진 및 설명글 올리기</button>
            <button class="upload-button" onclick="window.location.hash='noticesmattour'">맛투어 공지사항</button>
...
function setupEventListeners() {
    ...
    document.getElementById('uploadButton').addEventListener('click', () => {
        window.location.hash = 'newMATTOUR';
    });
}
...
async function loadRecentNotices() {
    ...
    try {
        const q = query(collection(db, "mattour-notices"), orderBy("isImportant", "desc"), orderBy("createdAt", "desc"), limit(5));
        ...
        querySnapshot.forEach((doc) => {
            ...
            a.href = `#noticesdetailsmattour?id=${doc.id}`;
        ...
        console.error("맛투어 공지사항을 불러오는 중 오류가 발생했습니다:", error);
    }
}

async function loadMatTourContent() {
    try {
        const mattourDoc = await getDoc(doc(db, "pages", "mattour"));
        if (mattourDoc.exists()) {
            const data = mattourDoc.data();
            document.getElementById('matTourImage').src = data.imageUrl;
            document.getElementById('matTourDescription').textContent = data.description || '설명글이 아직 등록되지 않았습니다.';
        } else {
            console.error("맛투어 데이터가 존재하지 않습니다.");
        }
    } catch (error) {
        console.error("맛투어 데이터를 불러오는 중 오류가 발생했습니다:", error);
    }
}

async function loadMatTourPrograms() {
    const programsContainer = document.getElementById('programsContainer');
    programsContainer.innerHTML = '';

    try {
        const programsQuery = query(collection(db, "programs"), where("title", ">=", "맛투어"), where("title", "<=", "맛투어\uf8ff"));
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (!programData.photoUrl) {
                console.warn(`프로그램에 대한 photoUrl이 없습니다: ${programData.title}`);
                programData.photoUrl = 'placeholder.jpg';  // 기본 이미지 설정
            }

            displayProgram({ ...programData, id: doc.id }, programsContainer);
        });
    } catch (error) {
        console.error("맛투어 프로그램을 불러오는 중 오류 발생:", error);
    }
}


async function loadMatTourReviews() {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            if (!reviewData.photoUrl) {
                console.warn(`후기에 대한 photoUrl이 누락되었습니다: ${reviewData.title}`);
                reviewData.photoUrl = 'placeholder.jpg';
            }

            displayReview({ ...reviewData, id: doc.id }, reviewsContainer);
        });
    } catch (error) {
        console.error("맛투어 후기를 불러오는 중 오류가 발생했습니다:", error);
    }
}
...
// 스타일 동적 추가
export function addMATTOURStyles() {
    const style = document.createElement('style');
    style.id = 'MATTOUR-styles';
    style.textContent = `
body {
...
// 스타일 제거
export function removeMATTOUR() {
    const style = document.getElementById('MATTOUR-styles');
    if (style) {
        style.remove();
    }
}