...
export function rendereditnoticesmattour() {
    // HTML 콘텐츠 렌더링
    ...
    setupEventListeners();
    loadNoticeForEdit();
    addeditnoticesmattourStyles(); // 스타일 추가
}
...
function setupEventListeners() {
    document.getElementById('editNoticeForm').addEventListener('submit', async (e) => {
        ...
        await updateDoc(doc(db, "mattour-notices", noticeId), {
            title: title,
            content: content,
        ...
        alert('공지사항이 수정되었습니다.');
        // 수정 후 noticesmattour 페이지로 이동
        window.location.hash = `noticesmattour`;
    });
}
...
async function loadNoticeForEdit() {
    ...
    try {
        const noticeRef = doc(db, "mattour-notices", noticeId);
...
// 스타일 동적 추가
export function addeditnoticesmattourStyles() {
    const style = document.createElement('style');
    style.id = 'edit-notices-mattour-styles';
    style.textContent = `
        body {
...
// 스타일 동적 제거
export function removeeditnoticesmattour() {
    const style = document.getElementById('edit-notices-mattour-styles');
    if (style) {
        style.remove();
    }
}