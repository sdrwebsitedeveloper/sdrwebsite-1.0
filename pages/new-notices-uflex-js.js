...
export function rendernewnoticesuflex() {
    // HTML 콘텐츠 렌더링
    document.getElementById('app').innerHTML = `
        <div class="container">
            ...
            <form id="noticeForm">
                ...
                <div class="form-group">
                    <label for="importantNotice">중요공지 체크란</label>
    ...
    setupEventListeners();
    addnewnoticesuflexStyles(); // 스타일 추가
}
...
function setupEventListeners() {
    document.getElementById('noticeForm').addEventListener('submit', async (e) => {
        ...
        await addDoc(collection(db, "notices"), {
            ...
            isImportant: isImportant, // 중요공지 여부 저장
            createdAt: new Date() // 공지사항 생성 시간
        });

        alert('공지사항이 성공적으로 업로드되었습니다.');
        window.location.hash = 'noticesuflex'; // SPA 방식으로 이동
    });
}

// 동적 스타일 추가
export function addnewnoticesuflexStyles() {
    const style = document.createElement('style');
    style.id = 'new-notices-uflex-styles';
    style.textContent = `
        body {
...
// 동적 스타일 제거
export function removenewnoticesuflex() {
    const style = document.getElementById('new-notices-uflex-styles');
    if (style) {
        style.remove();
    }
}