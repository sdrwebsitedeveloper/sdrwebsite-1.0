// pages/notices-details-uflex.js
...
export function rendernoticesdetailsuflex() {
    // URL 해시를 통해 id를 추출
    ...
    setupEventListeners();
    addnoticesdetailsuflexStyles(); // 동적 스타일 추가
    loadNoticeDetails();
}
...
function setupEventListeners() {
    ...
    document.getElementById('editNoticeButton').addEventListener('click', () => {
        window.location.hash = `editnoticesuflex?id=${noticeId}`; // SPA 방식으로 이동
    });
...
// 동적 스타일 추가
export function addnoticesdetailsuflexStyles() {
    const style = document.createElement('style');
    style.id = 'notices-details-uflex-styles';
    style.textContent = `
        body {
...
// 동적 스타일 제거
export function removenoticesdetailsuflex() {
    const style = document.getElementById('notices-details-uflex-styles');
    if (style) {
        style.remove();
    }
}